...
(() => {
  ...
  const BAD = /(slot|gacor|judi|casino|togel|bet(?:88|365)?|scatter|maxwin|rtp\s*\d|depo(?:sit)?|jackpot|spin\s*(?:gratis|sekarang)|ratu\s*89|rusia\s*777|pentaslot|judi\s*89|mb88|koko\s*slot|gaza\s*88|sigacor|bm-88|adsterra|propellerads|popads|exoclick|doubleclick|googlesyndication|googleadservices|adservice|adserver|popunder|clickunder|smartlink|pushads|sponsored)/i;
  ...
  const ownText = el => ((el.getAttribute?.('alt') || '') + ' ' + (el.getAttribute?.('title') || '') + ' ' +
    (el.getAttribute?.('href') || '') + ' ' + (el.getAttribute?.('src') || '') + ' ' +
    ((el.tagName === 'A' || el.tagName === 'INS') ? (el.innerText || '') : '')).slice(0,1600);

  const removeSmallAdUnit = el => {
    if (!el || !el.parentNode) return;
    let target = el;
    const p = el.parentElement;
    if (p && p !== document.body && p !== document.documentElement) {
      const r = p.getBoundingClientRect();
      if (r.width >= 80 && r.height >= 28 && r.height <= 420 && p.children.length <= 3) target = p;
    }
  ...
  const cleanTrustedVisibleAds = () => {
    ...
    try {
      document.querySelectorAll('a[href],img[src],iframe[src],ins').forEach(el => {
        const r = el.getBoundingClientRect();
        if (r.width < 30 || r.height < 20) return;
        const a = el.tagName === 'A' ? el : el.closest?.('a[href]');
        const href = a?.href || '';
        const src = el.getAttribute?.('src') || '';
        const signal = ownText(el) + ' ' + ownText(a || el);
        const explicitAd = BAD.test(signal) || suspicious(href) || suspicious(src);
        if (explicitAd) removeSmallAdUnit(a || el);
      });
    } catch (_) {}
  };
  ...
  const cleanGeneric = () => {
    ...
    try {
      document.querySelectorAll('img[src],iframe[src],script[src]').forEach(el => {
        const src = el.getAttribute?.('src') || '';
        if (src && (BAD.test(src) || suspicious(src))) removeSmallAdUnit(el);
      });
    } catch (_) {}
  };
  ...
  const clean = () => {
    try { document.querySelectorAll(selectors.join(',')).forEach(el => el.remove()); } catch (_) {}
    if (TRUSTED_SITE) cleanTrustedVisibleAds(); else cleanGeneric();
  };
  ...
  if (!window.__ricAggressiveObserver) {
    ...
    window.__ricAggressiveObserver.observe(document.documentElement, {
      childList: true,
      subtree: true,
      attributes: true,
